...
try:
    # DistillationRunner exists only in rsl_rl >= 3.x. It is optional and
    # unused for the OnPolicyRunner-based locomotion tasks (e.g. G1 Velocity).
...
def main():
    ...
    # `class_name` exists on the runner cfg only in isaaclab_rl >= 0.4 (rsl_rl 3.x).
    # On the 2.2 stack it is absent; the locomotion tasks use OnPolicyRunner.
